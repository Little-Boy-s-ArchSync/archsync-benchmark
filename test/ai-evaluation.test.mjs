...
function repairObservation(overrides = {}) {
  const row = {
    candidate_id: "c1",
    attempted: true,
    candidate_status: "VERIFIED_FOR_REVIEW",
    verification: {
      ...
      isolation_attestation_sha256: hash("9"),
    },
    execution: {
      isolation_status: "APPROVED",
      attestation_sha256: hash("9"),
      project_test_process_spawned: true,
    },
  };
  return {
    ...row,
    ...overrides,
    verification: { ...row.verification, ...overrides.verification },
    execution: { ...row.execution, ...overrides.execution },
  };
}

test("repair metrics require canonical Guardian isolation and keep TEST_ONLY inconclusive", () => {
  const result = calculateRepairMetrics([
    ...
    repairObservation({
      ...
      execution: {
        isolation_status: "TEST_ONLY",
        attestation_sha256: hash("8"),
        project_test_process_spawned: false,
      },
    }),
  ...
  for (const invalid of [
    repairObservation({ candidate_status: "unknown" }),
    { ...repairObservation(), verification: null },
    repairObservation({ verification: { decision: "unknown" } }),
    repairObservation({ verification: { tests: "unknown" } }),
    repairObservation({ verification: { conformance: "unknown" } }),
    repairObservation({ verification: { safe_apply: "yes" } }),
    repairObservation({ verification: { new_blocking_findings: -1 } }),
    repairObservation({ verification: { filesystem_isolation: "unknown" } }),
    repairObservation({ verification: { isolation_attestation_sha256: "bad" } }),
    { ...repairObservation(), execution: null },
    repairObservation({ execution: { isolation_status: "unknown" } }),
    repairObservation({ execution: { project_test_process_spawned: "yes" } }),
    repairObservation({ execution: { attestation_sha256: "bad" } }),
  ]) assert.throws(() => calculateRepairMetrics([invalid]), /canonical Guardian/u);

  for (const weakened of [
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "INCONCLUSIVE", filesystem_isolation: "not-approved", isolation_attestation_sha256: null }, execution: { isolation_status: "TEST_ONLY" } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "REJECT_TEST", filesystem_isolation: "not-approved", isolation_attestation_sha256: null }, execution: { isolation_status: "TEST_ONLY", project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "INCONCLUSIVE", isolation_attestation_sha256: null }, execution: { isolation_status: "TEST_ONLY", project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "INCONCLUSIVE", filesystem_isolation: "not-approved" }, execution: { isolation_status: "TEST_ONLY", project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "INCONCLUSIVE", filesystem_isolation: "not-approved", isolation_attestation_sha256: null }, execution: { isolation_status: "TEST_ONLY", project_test_process_spawned: true } }),
    repairObservation({ candidate_status: "VERIFIED_FOR_REVIEW", verification: { decision: "REJECT_UNSAFE", filesystem_isolation: "not-approved", isolation_attestation_sha256: null }, execution: { isolation_status: "REJECTED", attestation_sha256: null, project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "REJECT_UNSAFE", filesystem_isolation: "approved", isolation_attestation_sha256: hash("9") }, execution: { isolation_status: "REJECTED", attestation_sha256: null, project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "REJECT_UNSAFE", filesystem_isolation: "not-approved" }, execution: { isolation_status: "REJECTED", attestation_sha256: null, project_test_process_spawned: false } }),
    repairObservation({ candidate_status: "PROPOSED", verification: { decision: "REJECT_UNSAFE", filesystem_isolation: "not-approved", isolation_attestation_sha256: null }, execution: { isolation_status: "REJECTED", attestation_sha256: null, project_test_process_spawned: true } }),
    repairObservation({ verification: { isolation_attestation_sha256: hash("7") } }),
    repairObservation({ execution: { attestation_sha256: null } }),
    repairObservation({ execution: { attestation_sha256: hash("7") } }),
    repairObservation({ execution: { project_test_process_spawned: false } }),
  ]) assert.throws(() => calculateRepairMetrics([weakened]), /isolation boundary|acceptance invariant/u);

  for (const weakened of [
    repairObservation({ candidate_status: "PROPOSED" }),
    repairObservation({ verification: { decision: "REJECT_TEST" } }),
    repairObservation({ verification: { tests: "fail" } }),
    repairObservation({ verification: { conformance: "fail" } }),
    repairObservation({ verification: { safe_apply: false } }),
    repairObservation({ verification: { new_blocking_findings: 1 } }),
  ]) assert.throws(() => calculateRepairMetrics([weakened]), /acceptance invariant/u);

  assert.deepEqual(calculateRepairMetrics([repairObservation({
    candidate_status: "PROPOSED",
    verification: {
    ...
    attempted_repair_success_rate: 0,
    regressions: 0,
    regression_rate: 0,
    inconclusive: 0,
  });
