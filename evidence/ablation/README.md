...
Completed evidence must bind the frozen design and STUDY-103 dataset, result/metric hashes and analysis-code commit. Two clean metric replays must both match the declared `results.metrics_sha256`; two equal hashes for another artifact do not establish reproduction. `post_outcome_changes` must remain empty. `pnpm ablation:verify` validates the honest pending template. `pnpm ablation:gate` intentionally fails with `ABLATION_GATE_INCOMPLETE`; no condition result or approval is fabricated here.
